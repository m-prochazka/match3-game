...
window.addEventListener('resize', resizeCanvas);
    ...
    // Timer bonuses
    const TIMER_BONUS_SMALL = 0.5;          // seconds added for small matches
    const TIMER_BONUS_MEDIUM = 1;         // seconds added for medium matches
    const TIMER_BONUS_LARGE = 1.5;          // seconds added for large matches
    const TIMER_BONUS_SPECIAL = 2;        // seconds added for special tile activation
   ...
   function showTimerBonus(seconds, x, y) {
      const bonusX = x * CELL_SIZE + CELL_SIZE / 2;
      const bonusY = TOPBAR_HEIGHT + y * CELL_SIZE + CELL_SIZE / 2;
      timerBonuses.push({
        text: `+${seconds}s`,
...
function getCell(e) {
  ...
  const localX = (e.clientX - rect.left) * scaleX - currentShakeX;
  const localY = (e.clientY - rect.top) * scaleY - currentShakeY - TOPBAR_HEIGHT;
...
function getCellFromClient(clientX, clientY) {
  ...
  const localX = (clientX - rect.left) * scaleX - currentShakeX;
  const localY = (clientY - rect.top) * scaleY - currentShakeY - TOPBAR_HEIGHT;
  const x = Math.floor(localX / CELL_SIZE);
  const y = Math.floor(localY / CELL_SIZE);
  return { x, y };
}
...
for (const g of runs) {
  let timerBonusAdded = false; // Track if we've added timer bonus for this group
  ...
  for (const pos of g.positions) {
    ...
    if (!t.special) {
      ...
      // Add time bonus based on match size - ONLY ONCE PER GROUP
      if (!timerBonusAdded) {
        if (g.positions.length === 3) {
          addTime(TIMER_BONUS_SMALL);
          // Show notification at the center of the match group
          const centerX = Math.round(g.positions.reduce((sum, p) => sum + p.x, 0) / g.positions.length);
          const centerY = Math.round(g.positions.reduce((sum, p) => sum + p.y, 0) / g.positions.length);
          showTimerBonus(TIMER_BONUS_SMALL, centerX, centerY);
        } else if (g.positions.length === 4) {
          addTime(TIMER_BONUS_MEDIUM);
          const centerX = Math.round(g.positions.reduce((sum, p) => sum + p.x, 0) / g.positions.length);
          const centerY = Math.round(g.positions.reduce((sum, p) => sum + p.y, 0) / g.positions.length);
          showTimerBonus(TIMER_BONUS_MEDIUM, centerX, centerY);
        } else if (g.positions.length >= 5) {
          addTime(TIMER_BONUS_LARGE);
          const centerX = Math.round(g.positions.reduce((sum, p) => sum + p.x, 0) / g.positions.length);
          const centerY = Math.round(g.positions.reduce((sum, p) => sum + p.y, 0) / g.positions.length);
          showTimerBonus(TIMER_BONUS_LARGE, centerX, centerY);
        }
        timerBonusAdded = true; // Mark that we've added the bonus for this group
      }
    }
...
canvas.addEventListener('click', e => {
  ...
  // For mobile, use a simpler approach without drag distance checks
  const now = Date.now();
  const isDoubleTap = now - lastTapTime < 300;
  lastTapTime = now;
  
  // Prevent triggering on drags (mobile friendly version)
  if (isDrag && !isDoubleTap) return;
...
canvas.addEventListener('touchend', e => {
  ...
  const c = getCellFromClient(touch.clientX, touch.clientY);
  const t = grid[c.y]?.[c.x];
...
// Prevent default touch behaviors but allow special tile activation
document.addEventListener('touchstart', function(e) {
  if (e.target === canvas || e.target.closest('#gameWrapper')) {
    // Only prevent default if not targeting a special tile
    const rect = canvas.getBoundingClientRect();
    ...
    const scaleX = canvas.width / rect.width;
    const scaleY = canvas.height / rect.height;
    const gameX = Math.floor((x * scaleX) / CELL_SIZE);
    const gameY = Math.floor((y * scaleY - TOPBAR_HEIGHT) / CELL_SIZE);
    
    // Check if tapping on a special tile
    const t = grid[gameY]?.[gameX];
    if (!t || !t.special) {
      e.preventDefault();
    }
  }
